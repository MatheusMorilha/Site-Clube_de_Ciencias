async function Laboratorios() {
    try {
        var laboratorios = await fetch("https://ko6qqthj.api.sanity.io/v2021-10-21/data/query/production?query=*%5B_type+%3D%3D+%27laboratorios%27%5D+%7C+order%28_createdAt%C2%A0asc%29%7B%0A++titulo%2C%0A++descricao%2C%0A++%22imagem%22%3Aimagem%5B%5D.asset-%3Eurl%0A%7D%0A", {
            method: "GET"
        });

        var respostaEmJsonLab = await laboratorios.json();
        var resultadoAPILab = respostaEmJsonLab.result;

        const criarDivCards = document.querySelector(".cards");

        for (let i = 0; i < resultadoAPILab.length; i++) {
            const laboratorios = resultadoAPILab[i];
